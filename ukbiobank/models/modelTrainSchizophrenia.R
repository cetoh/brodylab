...
plot(mod_perf, type='roc', main=paste("Schizophrenia 64-split X Chromosome CSLV Model Performance: AUC = ", auc))
...
list(leader) %>% 
  ...
  xlab('False Positive Rate')+
  ylab('True Positive Rate')+
  ggtitle('ROC Curve for 64-Split X Chr Schizophrenia Model')+
  annotate("text", x = .75, y = .25, label = paste("AUC =", auc)) + 
  theme_bw() + theme(plot.title = element_text(size = 18, face = "bold")) +
  scale_color_npg()
...
metrics %>%
  ggplot(aes(recall,precision)) + 
  geom_line() +
  theme_minimal() + ggtitle("Schizophrenia 64-split X Chr CSLV Model Precision-Recall") +
  theme_bw() + theme(plot.title = element_text(size = 18, face = "bold")) +
  scale_color_npg()
